...
and the `release.yml` workflow, so authentication and provenance both come from
the workflow's OIDC identity and no npm token exists. A new package has to be
registered the same way before its first automated publish.
